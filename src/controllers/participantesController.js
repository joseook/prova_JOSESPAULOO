...
export const createParticipante = (req, res) => {
    ...
    mysqlPool.query("SELECT email, cpf FROM usuarios WHERE email = ? and cpf = ?", [email, cpf], (err, result) => {
        if (err) return handleError(res, err, "Erro ao criar participante.");
        if (result.length > 0) {
            return res.status(409).json({message: "Já existe um participante cadastrado com esse email ou CPF."})
        }
        ...
        mysqlPool.query(
            query,
            [id, nome, email, telefone, imagem, cpf], (err, result) => {
                if(err) return handleError(res, err, "Erro ao tentar criar um participante");
                res.status(201).json({message: "Pariticpante criado com sucesso", id})
            }
        )
    });
};
