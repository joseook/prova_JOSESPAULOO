...
export const createPalestrante = async (req, res) => {
    ...
    mysqlPool.query("SELECT email from palestrantes WHERE email = ?", [email], (err, results) => {
        if (err) return handleError(res, err, "Erro ao criar palestrante.");
        if (results.length > 0) {
            res.status(409).json({message: "O email do palestrante já existe na base de dados!. Tente outro email."})
        }
        ...
        mysqlPool.query(query,
                [id, nome, email, telefone, empresa, expertise, trabalhos_recentes, imagem], (err, result) => {
                    if(err) return handleError(res, err, "Erro ao tentar criar um palestrante!.")
...
export const getPalestrantes = (req, res) => {
    const query = "SELECT nome, email, telefone, empresa, expertise, trabalhos_recentes FROM palestrantes";
